...
function typhoon() {
    ...
    xmlhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            document.getElementById("typhoon_title").textContent = "[제 " + myObj.response.body.items.item[number].typSeq + "호 태풍 " + myObj.response.body.items.item[number].typName + "에 관한 기상청 태풍정보 제 " + myObj.response.body.items.item[number].tmSeq + "호]"
            ...
            document.getElementById("typhoon_info_text").textContent =
                "제 " + myObj.response.body.items.item[number].typSeq + "호 태풍 '" + myObj.response.body.items.item[number].typName + "'은(는) " + myObj.response.body.items.item[number].typLoc + "에서 " + myObj.response.body.items.item[number].typDir + "방향으로 " + myObj.response.body.items.item[number].typSp + "km/h의 속도로 이동중.";
